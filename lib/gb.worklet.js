...
function songContext(registerWrite, enableChannel, disableChannel) {
	...
	function tick() {
		trax.forEach(tickTrack);
	}
	
	function tickTrack(s, i) {
		while (s.timer <= 0 && s.idx >= 0) {
			...
			if (op === 0xB3) { // gameboy apu register write
				...
				registerWrite(i, reg, val);
				if (val & 0x80) {
					if      (reg === 0x04 && s.mask[0]) enableChannel(i, 0);
					else if (reg === 0x09 && s.mask[1]) enableChannel(i, 1);
					else if (reg === 0x0E && s.mask[2]) enableChannel(i, 2);
					else if (reg === 0x13 && s.mask[3]) enableChannel(i, 3);
				}
			} else if (op === 0x61) {
