    ...
    this.port.onmessage = ({ data: e }) => {
			if (e.type === 'play') {
				...
				exportsPromise.then(GB => {
					...
					this.lchan = new Int32Array(GB.memory.buffer, GB.lchan, 128);
					this.rchan = new Int32Array(GB.memory.buffer, GB.rchan, 128);
  ...
  process (inputs, outputs, parameters) {
		if (this.updateGB) {
			...
			for (let outBufferOffset = 0; outBufferOffset < out1.length; outBufferOffset += 128) {
				...
				for (let i = 0; i < 128; ++i) {
					out1[i+outBufferOffset] = this.lchan[i] / 0x10000;
					out2[i+outBufferOffset] = this.rchan[i] / 0x10000;
				}
			}
		}
		if (this.generateSamples) this.generateSamples(outputs);
		return true
  }
}
